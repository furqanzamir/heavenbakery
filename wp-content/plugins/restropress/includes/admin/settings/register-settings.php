...
function rpress_get_registered_settings() {
	...
	$rpress_settings = array(
			...
			array(
				...
				'sale_notifications' => array(
					'sale_notification_subject' => array(
						...
						'name' => __( 'Sale Notification Subject', 'restro-press' ),
						'desc' => __( 'Enter the subject line for the sale notification email.', 'restro-press' ),
						'type' => 'text',
						'std'  => 'New fooditem purchase - Order #{payment_id}',
					),
					'sale_notification_heading' => array(
						'id'   => 'sale_notification_heading',
						'name' => __( 'Sale Notification Heading', 'restro-press' ),
						'desc' => __( 'Enter the heading for the sale notification email.', 'restro-press' ),
						'type' => 'text',
						'std'  => __( 'New Sale!', 'restro-press' ),
					),
					'sale_notification' => array(
						'id'   => 'sale_notification',
						'name' => __( 'Sale Notification', 'restro-press' ),
						'desc' => __( 'Enter the text that is sent as sale notification email after completion of a purchase. HTML is accepted. Available template tags:', 'restro-press' ) . '<br/>' . rpress_get_emails_tags_list(),
						'type' => 'rich_editor',
						'std'  => rpress_get_default_sale_notification_email(),
					),
					'admin_notice_emails' => array(
						'id'   => 'admin_notice_emails',
						'name' => __( 'Sale Notification Emails', 'restro-press' ),
						'desc' => __( 'Enter the email address(es) that should receive a notification anytime a sale is made, one per line.', 'restro-press' ),
						'type' => 'textarea',
						'std'  => get_bloginfo( 'admin_email' ),
					),
					'disable_admin_notices' => array(
						...
						'desc' => __( 'Check this box if you do not want to receive sales notification emails.', 'restro-press' ),
						'type' => 'checkbox',
					),
				),
			)
		...
		'notification' => apply_filters('rpress_settings_notification',
			array(
				'active_notification' => array(
					'notification_header' => array(
						'id'   => 'button_header',
						'name' => '<strong>' . __( 'Notification Settings', 'restro-press' ) . '</strong>',
						'desc' => __( 'Options for configure notifications', 'restro-press' ),
						'type' => 'header',
					),
		...
		/** Delivery Settings */
		'delivery' => apply_filters('rpress_settings_delivery',
			array(
				'main' => array(
					'enable_delivery' => array(
						...
						'type'          => 'checkbox',
						'tooltip_title' => __( 'Enable delivery options', 'restro-press' ),
					),
					'enable_pickup' => array(
						'id'            => 'enable_pickup',
						...
						'type'          => 'checkbox',
						'tooltip_title' => __( 'Enable pickup options', 'restro-press' ),
					),
					'open_time' => array(
						'id'            => 'open_time',
...
function rpress_get_settings_tabs() {
	...
	$tabs['notification'] = __( 'Order Notification', 'restro-press' );

	$tabs['delivery']     = __( 'Delivery Options', 'restro-press' );
...
function rpress_get_registered_settings_sections() {
	...
	$sections = array(
		...
		'emails'     => apply_filters( 'rpress_settings_sections_emails', array(
			...
			'sale_notifications' => __( 'New Sale Notifications', 'restro-press' ),
		) ),
		...
		'delivery'   => apply_filters( 'rpress_settings_sections_delivery', array(
			'main'               => __( 'General', 'restro-press' ),
		) ),
			...
			if ( false === $license->success ) {
				...
				switch( $license->error ) {
					...
					default :
						...
						$messages[] = sprintf( __( 'There was an error with this license key: %s. Please <a href="%s">contact our support team</a>.', 'restro-press' ), $error, 'https://fooditems.com/support' );
					...
					default:
						...
						if( 'lifetime' === $license->expires ) {
							...
							$messages[] = sprintf(
								...
								date_i18n( get_option( 'date_format' ), strtotime( $license->expires, current_time( 'timestamp' ) ) ),
								'https://fooditems.com/checkout/?rpress_license_key=' . $value . '&utm_campaign=admin&utm_source=licenses&utm_medium=renew'
							);
